...
import Theme.Theme -- uses Xresources
...
myFontBig = "xft:Inconsolata Nerd Font Mono:weight=regular:pixelsize=60:antialias=true:hinting=true" -- myFont defined in Theme.Theme
...
myStartupHook = do
  ...
  spawn "sleep 2 && trayer --edge top --align right --width 5 --SetDockType true --SetPartialStrut true --expand true --monitor 0 --transparent true --alpha 0 --tint 0x282A36 --height 24"
  setWMName "LG3D"
...
myKeys =
  [ ("M-<Return>", spawn myTerminal),
    ...
    ("M-d", spawn ("dmenu_run -p 'Run: ' -h 22 -nb '#282A36' -sb '" ++ color02 ++ "' -sf '" ++ colorBack ++ "'" ++ " -fn 'JetBrainsMono Nerd Font:pixelsize=12'")),
    --    , ("M-e", spawn myDmenu)
    ...
    ("M-f", selectWindow emConfig{txtCol="Green"} >>= (`whenJust` windows . W.focusWindow)),
    ("M-c", selectWindow emConfig{txtCol="Red"} >>= (`whenJust` killWindow)),
    -- Workspace Movement
    ("M-<Tab>", nextWS),
    ("M-S-<Tab>", prevWS),
    ("M-C-Right", nextWS),
    ...
    ("M-p s", spawn "sshmenu"),
    ("M-z s", sshPrompt myPromptConfig),
    ("M-z z", zshPrompt myPromptConfig "/home/eddie/.config/xmonad/scripts/capture.zsh"),
    ("M-z a", xmonadPrompt myPromptConfig),
    ("M-z p", passPrompt myPromptConfig),
    ("M-z g", passGeneratePrompt myPromptConfig),
...
--    ("S-M1-k", spawn "rofi -show linkding -modi linkding:$HOME/.local/bin/rofi-linkding"),
    ...
    ("M-w",    spawn myBrowser),
    ("M-v",    spawn "urxvt -name 'vifm' -e vifm"),
