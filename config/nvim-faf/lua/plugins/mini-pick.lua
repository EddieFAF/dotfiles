return {
  ...
  config = function()
    ...
    map('n', '<leader>mf', MiniPick.builtin.files, { desc = 'Pick: Find files in (cwd)' })
    map('n', '<leader>mg', MiniPick.builtin.grep_live, { desc = 'Pick: live grep (cwd)' })
    map('n', '<leader>mb', MiniPick.builtin.buffers, { desc = 'Pick: show open buffers' })
